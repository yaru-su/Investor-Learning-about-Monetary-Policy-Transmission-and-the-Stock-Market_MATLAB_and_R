...
DataRPMartinMonthly <- DataMartinDaily %>%
  group_by(month = floor_date(date, "month")) %>%  # Group data by month (round each date down to the first day of its month)
  summarise(across(-date, last, .names = "{.col}")) %>%  # Within each month, keep the last available daily observation for all columns (except 'date')
  ungroup() %>%
  rename(date = month)
